...
### Safety valve: the skip list (v0.6.1+)
...
Match is **exact-basename, ASCII case-insensitive** — `"target"`
skips `target/` and `Target/` but not `my-target-files/`. The
skip list governs **prefetch only**; a user click on a skipped
